...
public class PageTitleTests extends TestCase {
    ...
    public void testFromInternalLink() throws Exception {
        ...
        Assert.assertEquals(enwiki.titleForInternalLink("/wiki/India").getPrefixedText(), "India");
        assertNull(enwiki.titleForInternalLink("/wiki/India").getNamespace());

        Assert.assertEquals(enwiki.titleForInternalLink("/wiki/Talk:India").getNamespace(), "Talk");
        Assert.assertEquals(enwiki.titleForInternalLink("/wiki/Talk:India").getText(), "India");
        Assert.assertEquals(enwiki.titleForInternalLink("/wiki/Talk:India").getFragment(), null);

        Assert.assertEquals(enwiki.titleForInternalLink("/wiki/Talk:India#").getNamespace(), "Talk");
        Assert.assertEquals(enwiki.titleForInternalLink("/wiki/Talk:India#").getText(), "India");
        Assert.assertEquals(enwiki.titleForInternalLink("/wiki/Talk:India#").getFragment(), "");

        Assert.assertEquals(enwiki.titleForInternalLink("/wiki/Talk:India#History").getNamespace(), "Talk");
        Assert.assertEquals(enwiki.titleForInternalLink("/wiki/Talk:India#History").getText(), "India");
        Assert.assertEquals(enwiki.titleForInternalLink("/wiki/Talk:India#History").getFragment(), "History");
    }

    public void testCanonicalURL() throws Exception {
        Site enwiki = new Site("en.wikipedia.org");

        Assert.assertEquals(enwiki.titleForInternalLink("/wiki/India").getCanonicalUri(), "https://en.wikipedia.org/wiki/India");
        Assert.assertEquals(enwiki.titleForInternalLink("/wiki/India Gate").getCanonicalUri(), "https://en.wikipedia.org/wiki/India_Gate");
        Assert.assertEquals(enwiki.titleForInternalLink("/wiki/India's Gate").getCanonicalUri(), "https://en.wikipedia.org/wiki/India%27s_Gate");
    }

    public void testSite() throws Exception {
        Site enwiki = new Site("en.wikipedia.org");

        Assert.assertEquals(enwiki, new PageTitle(null, "Test", enwiki).getSite());
